 ...
 */
/*
 * Indentation is done via the .clang-format at the root of the repo
 * and the following command:
 * clang-format -style=file -assume-filename=.clang-format -i src/hostapi/sndio/pa_sndio.c
 */
#include <errno.h>
#include <poll.h>
#include <pthread.h>
...
static void *sndioThread( void *arg )
{
    ...
    while( !sndioStream->stopped )
    {
        ...
        PaUtil_BeginBufferProcessing( &sndioStream->bufproc, &timeinfo, 0 );
        if( sndioStream->mode & SIO_PLAY )
        {
            PaUtil_SetOutputFrameCount( &sndioStream->bufproc, sndioStream->par.round );
            PaUtil_SetInterleavedOutputChannels( &sndioStream->bufproc, 0, sndioStream->wbuf,
                                                 sndioStream->par.pchan );
        }
        if( sndioStream->mode & SIO_REC )
        {
            PaUtil_SetInputFrameCount( &sndioStream->bufproc, sndioStream->par.round );
            PaUtil_SetInterleavedInputChannels( &sndioStream->bufproc, 0, sndioStream->rbuf,
                                                sndioStream->par.rchan );
        }
        result = paContinue;
        n = PaUtil_EndBufferProcessing( &sndioStream->bufproc, &result );
                           ...
                           void *userData )
{
    ...
    PA_DEBUG( ( "inputChannelCount = %d, outputChannelCount = %d, ifmt = %x, ofmt = %x\n", inputChannelCount,
                outputChannelCount, ifmt, ofmt ) );
    err = PaUtil_InitializeBufferProcessor( &sndioStream->bufproc, inputChannelCount, ifmt, siofmt, outputChannelCount,
                                            ofmt, siofmt, sampleRate, streamFlags, framesPerBuffer, par.round,
    ...
    sndioStream->base.streamInfo.outputLatency =
        ( mode & SIO_PLAY )
            ? (double)( par.bufsz + PaUtil_GetBufferProcessorOutputLatencyFrames( &sndioStream->bufproc ) ) /
                  (double)par.rate
            : 0;
...
static PaError BlockingReadStream( PaStream *paStream, void *data, unsigned long numFrames )
{
    ...
    while( numFrames > 0 )
    {
        ...
        sndioStream->rpos += n;
        PaUtil_SetInputFrameCount( &sndioStream->bufproc, n );
        PaUtil_SetInterleavedInputChannels( &sndioStream->bufproc, 0, sndioStream->rbuf, sndioStream->par.rchan );
        res = PaUtil_CopyInput( &sndioStream->bufproc, &data, n );
        if( res != n )
        {
...
static PaError BlockingWriteStream( PaStream *paStream, const void *data, unsigned long numFrames )
{
    ...
    while( numFrames > 0 )
    {
        ...
        if( n > numFrames )
            n = numFrames;
        PaUtil_SetOutputFrameCount( &sndioStream->bufproc, n );
        PaUtil_SetInterleavedOutputChannels( &sndioStream->bufproc, 0, sndioStream->wbuf, sndioStream->par.pchan );
        res = PaUtil_CopyOutput( &sndioStream->bufproc, &data, n );
        if( res != n )
        {
...
static PaError StartStream( PaStream *paStream )
{
    ...
    sndioStream->wpos = 0;
    sndioStream->rpos = 0;
    PaUtil_ResetBufferProcessor( &sndioStream->bufproc );
...
static PaError CloseStream( PaStream *paStream )
{
    ...
    PaUtil_TerminateBufferProcessor( &sndioStream->bufproc );
    PaUtil_FreeMemory( sndioStream );
    return paNoError;
}
